...
\Procedure{fuerza\_bruta}
    {\texttt{list<item>} items, \texttt{int } W, \texttt{mochila } m,
      \texttt{item\_sum } solucion}
\If {vacia(items)}
\If  {peso\_incl(m) $\leq$ W \textbf{and}
        beneficio\_excl(m) $<$ beneficio\_excl(solucion)}
\State solucion $\gets$ m
\EndIf
\Else 
\State m\_con\_iesimo $\gets$ agregar\_item(first(items), m)
\State m\_sin\_iesimo $\gets$ no\_agregar\_item(first(items), m)
\State fuerza\_bruta(siguientes(items), W, m\_sin\_iesimo, solucion)
\State fuerza\_bruta(siguientes(items), W, m\_con\_iesimo, solucion)
\EndIf
\EndProcedure
\end{algorithmic}
...
Así se obtienen $A$ y $B$, ~vectors~ que representarán los
subconjuntos de los conjuntos de partes de cada una de las dos mitades
de $S$. Cada uno de los elementos de los ~vector~ $A$ y $B$ es una
posible carga de la mochila. Son subconjuntos de los conjuntos de
partes de los elementos de las dos mitades del conjunto de items, y no
conjuntos de partes propiamente dichos, ya que si alguna de las sumas
acumula un peso mayor a $W$, entonces podemos excluirla sin riesgo a
dejar de lado la solución buscada. 

Luego se ordena el ~vector~ $B$ tanto por peso como por beneficio, en
orden creciente. Para ello se ordena, en primer lugar, por peso. Esta
operación se lleva a cabo utilizando el algoritmo ~sort~ de la
biblioteca de ~c++~, el cual tiene una complejidad de $O(l\log{l})$, con
$l$ el tamaño de la entrada. Como en nuestro caso el tamaño de la
entrada está acotado superiormente por $2^{\frac{n}{2}}$, entonces
este llamado tiene una complejidad
$$ O(2^{\frac{n}{2}} \log{2^{\frac{n}{2}}}) = O(n \times 2^{\frac{n}{2}})$$.
...
reresentn ya soluciones al problema, es obvio que como lo que se
...
\Function{meet\_in\_the\_middle}
    {\texttt{vector<item> }items, \texttt{int }W}
...
\begin{algorithmic}[1]
\State {vector<item\_sum>}
\Function{filtrar\_mochilas\_pesadas}
{\texttt{vector<item\_sum> } mochilas }}
\State res $\gets$ [(0,0)]
\ForAll {m $\in$ mochila} 
    \If {res.back().beneficio < m.beneficio}
        { res.push\_back(m)}
    \EndIf
\EndFor
